...
double matrix_multiplication_a(long a[N][N], long b[N][N], long c[N][N]) {
    ...
    for (int row {0}; row < N; row++) {
        for (int column {0}; column < N; column++) {
            for (int i {0}; i < N; i++) {
                c[row][column] += a[i][row] * b[i][column];
            }
        }
    }
    auto t2 = clock();
...
int main() {
    fill_matrix(a);
    fill_matrix(b);
    auto time = matrix_multiplication_a(a, b, c);
    std::cout << "Matrix multiplication (awful mem access) A^T*B=C took: " << time << " seconds. (" << c[4][3] << ")\n";
    return 0;
}